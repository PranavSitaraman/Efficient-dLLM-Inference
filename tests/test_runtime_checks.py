...
def test_moe_runtime_fallback_patch_is_idempotent(monkeypatch):
    import sys
    import types
    ...
    first = rc.ensure_vllm_moe_runtime(strict=True, verbose=False)
    second = rc.ensure_vllm_moe_runtime(strict=True, verbose=False)

    assert set(first["patched_fallback_ops"]) == {"moe_align_block_size", "moe_sum"}
    assert set(second["patched_fallback_ops"]) == {"moe_align_block_size", "moe_sum"}
    assert second["missing_required_ops"] == []
