...
from .tasks import build_prompt, extract_prompt_and_reference
...
def _save_checkpoint(path: str, policy: PhaseAV2Policy, soft_mask, metadata: dict) -> None:
    payload = {
        "policy": policy.state_dict(),
        "soft_mask": soft_mask.state_dict() if soft_mask is not None else {},
        "metadata": metadata,
    }
...
def train(cfg: dict) -> str:
    ...
    log_path = os.path.join(output_dir, "warmstart_log.jsonl")
        ...
        try:
            for epoch in range(epochs):
                ...
                for idx in pbar:
                    ...
                    with torch.no_grad():
                        _, trajectory = speculative_inference(
