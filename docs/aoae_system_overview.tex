...
\paragraph{Plan update (2026-04-13).}
The current speculative direction is now a \emph{lossy} $K_{\text{draft}}{>}1$ draft--validate loop rather than verifier-every-step speculation. The hard-routed auxiliary is intended to run with a plug-and-play cache/freezing policy, making it faster and lower-quality by construction; the slower verifier runs in normal inference mode, scores the drafted state with PRISM or a PRISM-like head, and may reject/remask stale regions or clear the drafter cache when disagreement spikes. In this plan, $\cK_{\mathrm{spec}}$ is a transient verifier frontier of drafted candidates, while $\cK_{\mathrm{stable}}$ remains a separate optional contribution that can be disabled in the initial implementation.
...
\State \textbf{Init:} $\by_T \leftarrow (\mask, \dots, \mask)$, \; $\cK_{\mathrm{spec}} \leftarrow \emptyset$, \; $\cK_{\mathrm{stable}} \leftarrow \emptyset$
\For{$t = T, T{-}1, \dots, 1$}
    \State \textbf{Phase 0 (Draft):} $\{d_t^k\} \leftarrow p_{\btheta}^{\text{hard}}(\cdot \mid \bx, \by_t)$; draft candidate tokens and optional draft-side KV state \Comment{${\sim}$1.4B active}
    \State \textbf{Phase 0b (Verify):} $\{p_t^k\} \leftarrow p_{\btheta}^{\text{soft}}(\cdot \mid \bx, \by_t;\, \cK_{\mathrm{spec}},\, \cK_{\mathrm{stable}})$; $\{q_t^k\} \leftarrow q_\psi(\by_t)$ \Comment{all 16B; validates transient $\cK_{\mathrm{spec}}$, persists only $\cK_{\mathrm{stable}}$}
    ...
    \For{$k : r_t^k {=} 1,\, m_t^k {=} 0$} \Comment{\textbf{Phase 1: Remask}}
        \State $y_{t-1}^k \leftarrow \mask$; \; $\cK_{\mathrm{stable}} \leftarrow \cK_{\mathrm{stable}} \setminus \{k\}$  \Comment{remask evicts from stable cache}
    \EndFor
    ...
    \EndFor
    \State $\cK_{\mathrm{spec}} \leftarrow \textsc{UpdateDraftFrontier}\bigl(\cK_{\mathrm{spec}}, \{(k, y_{t-1}^k) : u_t^k {=} 1,\, m_t^k {=} 1\}\bigr)$ \Comment{\textbf{Phase 3a: Speculative Frontier}---transient drafted-but-unverified positions; may accumulate across draft steps until the next verifier pass}
    \For{$k : \kappa_t^k {=} 1,\, r_t^k {=} 0$} \Comment{\textbf{Phase 3b: Stable Cache}---persistent; evicted only by remask (Phase~1)}
        \State $\cK_{\mathrm{stable}} \leftarrow \cK_{\mathrm{stable}} \cup \{k\}$
    \EndFor
\EndFor
...
At each step (\Cref{alg:aoae}), the auxiliary drafts candidate tokens; on verifier events, the primary validates the current transient speculative frontier $\cK_{\mathrm{spec}}$ while persisting only $\cK_{\mathrm{stable}}$; the policy outputs joint actions; remasks evict from $\cK_{\mathrm{stable}}$ (Phase~1); unmasks sample from a composed distribution (Phase~2); newly drafted tokens are added to the transient speculative frontier $\cK_{\mathrm{spec}}$ until the next verifier pass (Phase~3a); and the $\kappa_t$ head commits stably-predicted positions to $\cK_{\mathrm{stable}}$ (Phase~3b).
The two pools serve distinct roles: $\cK_{\mathrm{spec}}$ is a transient verifier frontier / speculative-accept state, not a persistent cache, while $\cK_{\mathrm{stable}}$ captures multi-step validity (the KV at this position is unlikely to drift as decoding continues).
In the current runtime, `primary_every_n` allows a lossy $k{>}1$ draft schedule: $\cK_{\mathrm{spec}}$ can accumulate provisional drafted positions across auxiliary-only steps until the next verifier event, while accepted tokens remain directly in $\by_t$ rather than being stored as a persistent cache.
...
The speculative loop in \Cref{sec:transition} maintains two separate state mechanisms with different lifetimes.
\emph{Speculative acceptance / verification frontier} ($\cK_{\mathrm{spec}}$, Phase~3a) is transient: it stores drafted candidates that the primary should validate on the next verification event, and it may accumulate across multiple auxiliary draft steps. Accepted tokens remain in the current sequence state rather than inside $\cK_{\mathrm{spec}}$, so this is still not a persistent cache.
\emph{Stable KV caching} ($\cK_{\mathrm{stable}}$, Phase~3b) is persistent: the $\kappa_t$ head commits positions whose KV vectors are predicted to remain accurate across \emph{multiple future steps}, evicting them only when a remask explicitly changes that position's context.
...
\paragraph{Reward.}
Let $\hat{T}$ be the completion step and $\bar{\kappa} = \frac{1}{TL}\sum_{t=1}^{T}|\cK_{\mathrm{stable},t}|$ the mean fraction of positions in the stable cache per step.
We define a compute-aware effective-FLOP fraction and speed factor:
\begin{equation}
    \mathrm{eff} \;=\; \frac{\hat{T}}{T}\bigl(1 - \bar{\kappa}\bigr),
    \qquad
    \mathrm{speed} \;=\; (1 - \mathrm{eff})^{\alpha}.
    \label{eq:speed}
\end{equation}
The full reward is:
\begin{equation}
    R \;=\; r(\by^*, \by_{\hat{T}}) \cdot \mathrm{speed}
      \;-\; \beta \sum_{t=1}^{T} \mathrm{Thrash}(t)
      \;-\; w_u\, f_{\mathrm{unres}}
      \;+\; w_{\mathrm{F1}}\, \overline{\mathrm{F1}}_{\mathrm{cache}}
      \;+\; w_q\, \mathrm{F1}_{\mathrm{access}},
    \label{eq:reward}
\end{equation}
where $r(\cdot)$ is task correctness (binary for math); $\mathrm{Thrash}(t) = \sum_k \ind[k \in \cK_{\mathrm{stable},t-1} \land r_t^k = 1]$ counts stable-cache evictions from remasking, penalized by $\beta \geq 0$; $f_{\mathrm{unres}}$ penalizes unresolved \mask{} tokens at completion; $\overline{\mathrm{F1}}_{\mathrm{cache}}$ is a per-step soft F1 rewarding $\kappa_t$ for committing low-drift positions to $\cK_{\mathrm{stable}}$; and $\mathrm{F1}_{\mathrm{access}}$ is a dense reward for the access head's next-$H$ predictions.
The compute-aware speed term jointly credits fewer steps \emph{and} cheaper steps: a policy that caches 80\% of positions for all $T$ steps still earns $\mathrm{speed} = (1 - 1.0 \cdot 0.2)^\alpha$ rather than zero as in a pure step-count formulation.
The multiplicative structure ensures incorrect generations receive no speed bonus regardless of efficiency, preventing reward hacking~\cite{jazbec2025learning}.
