...
With standard hard top-$k$ routing, LLaDA~2.1-mini activates ${\sim}$1.4B of 16B parameters per token (fast); with temperature-controlled widened routing, additional experts beyond the native top-$k$ can participate (slower but more expressive).
...
AOAE uses two routing modes of the same frozen model:
...
Replaces hard routing with temperature-controlled tail expansion over the
native LLaDA gate scores:
\begin{equation}
    s_i = \sigma(z_i), \qquad
    \mathcal{I}_{k} = \TopK_k\!\left(s_i + b_i\right), \qquad
    \mathcal{I}_{K_{\text{soft}}} = \TopK_{K_{\text{soft}}}\!\left(s_i + b_i\right),
    \label{eq:soft_route_selected_sets}
\end{equation}
\begin{equation}
    \tilde{w}_i(\tau_r) =
    \begin{cases}
        s_i, & i \in \mathcal{I}_k,\\
        \tau_r \, s_i, & i \in \mathcal{I}_{K_{\text{soft}}} \setminus \mathcal{I}_k,\\
        0, & \text{otherwise,}
    \end{cases}
    \qquad
    w_i^{\text{soft}} = \frac{\tilde{w}_i(\tau_r)}{\sum_j \tilde{w}_j(\tau_r)}.
    \label{eq:soft_route_selected}
\end{equation}
where $z_i$ are router logits, $b_i$ is the learned expert-bias term, and
$\tau_r > 0$ is the routing temperature. In implementation we preserve the
original bias-aware group-limited top-$k$ selection semantics, optionally
widening the active expert set to $K_{\text{soft}} \ge k$, and scale only the
\emph{additional} experts outside the native hard top-$k$. At any $\tau_r$ with
$K_{\text{soft}} = k$, this exactly recovers hard routing. As $\tau_r \to 0$,
the added experts vanish and the primary approaches the auxiliary; as $\tau_r$
increases toward $1$, the widened expert set participates more strongly.
In implementation we load a single shared backbone and toggle routing in-place, so the auxiliary and primary share frozen weights exactly and differ only in the routing function.
...
At $\tau_r = 0.01$, $>$99\% of routing weight remains on the native top-$k$ experts, yielding high agreement with modest quality improvement.
At $\tau_r = 0.1$, the widened tail begins to matter: richer predictions but lower agreement.
At $\tau_r = 0.5$, additional experts contribute substantially; quality may improve but the policy must be highly selective about caching.
...
Primary experiment: sweep $\tau_r \in \{0.0001, 0.001, 0.005, 0.01, 0.02, 0.05\}$.
For the routing-only POC1 sweep, we disable remasking to isolate the routing-temperature speed--quality effect.
...
\paragraph{Metrics.}
Accuracy (exact match / pass@1), throughput (tokens/sec on A100), NFEs, agreement rate $\bar{\alpha} = \frac{1}{TL}\sum_{t,k} \alpha_t^k$, cache-retention / keep rate, effective speedup vs.\ soft-routed-only baseline, refresh ratio $\frac{1}{T}\sum_t |\cQ_t|/L$, and boundary depth $\frac{1}{T}\sum_t \ell_t^\star$.
