...
In implementation, this frontier is swept with named operating points over verifier cadence, agreement threshold, unmask budget, remasking, and policy temperature rather than by changing policy temperature alone.
\end{abstract}
...
The operating frontier is controlled by the verifier mode, the verifier cadence, agreement threshold, unmask budget, remasking policy, drafter shortcut, draft horizon $K_{\text{draft}}$, policy temperature $\tau_\pi$, and composition strength $\gamma$.
...
\end{itemize}
In the current $K_{\text{draft}}{=}1$ implementation, $\cK_{\text{spec}}$ can be materialized as a one-step accepted/reusable mask; for $K_{\text{draft}}{>}1$, it should be interpreted only as a verifier frontier, not a persistent cache.
