...
echo "Quick start:"
echo "  1. Preflight:"
echo "     python3 -m aoae.cli preflight --config configs/default.yaml"
echo ""
echo "  2. Train PRISM adapter:"
echo "     python3 -m aoae.cli train --config configs/default.yaml --stage prism"
echo ""
echo "  3. Train AOAE policy via GRPO:"
echo "     torchrun --nproc_per_node 4 -m aoae.cli train --config configs/default.yaml --stage grpo"
echo ""
echo "  4. Evaluate:"
echo "     python3 -m aoae.cli eval --config configs/default.yaml --checkpoint outputs/default/policy_final.pt"
echo ""
echo "Multi-GPU note:"
echo "  Commands using configs with hardware.tp_size > 1 auto-relaunch under torchrun."
echo ""
echo "Paper / POCs:"
echo "  python3 -m aoae.cli paper-suite --config configs/paper.yaml --max_samples 100"
echo "  python3 -m aoae.cli tau-sweep --config configs/poc1.yaml --max_samples 100"
echo "  python3 -m aoae.cli reuse-sweep --config configs/poc2.yaml --max_samples 100"
echo ""
echo "SLURM cluster:"
echo "  sbatch slurm/train.sh prism configs/default.yaml"
echo "  sbatch slurm/train.sh grpo configs/default.yaml auto"
echo "  sbatch slurm/eval.sh configs/default.yaml outputs/default/policy_final.pt"
echo "  sbatch slurm/paper.sh suite configs/paper.yaml --max_samples 100"
echo ""
echo "For the large MoE runtime:"
echo "  Use configs/llada21_flash.yaml."
