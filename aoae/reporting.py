...
def load_comparison_rows(eval_path: Path) -> List[Dict[str, Any]]:
    ...
    for result in results:
        ...
        row = {
            ...
            "frontier_accept_rate": f"{_safe_float(result.get('frontier_accept_rate')):.6f}",
        ...
        rows.append(
            {
                ...
                "layer_drift_measure": summary.get("layer_drift_measure", "hidden_state_proxy"),
                "exact_kv_drift_steps": summary.get("exact_kv_drift_steps", 0.0),
                "hidden_state_proxy_steps": summary.get("hidden_state_proxy_steps", 0.0),
                "mean_layer_drift_slope": summary.get("mean_layer_drift_slope", 0.0),
                "attention_deviation_available": summary.get("attention_deviation_available", False),
                "mean_attention_deviation_slope": summary.get("mean_attention_deviation_slope", 0.0),
