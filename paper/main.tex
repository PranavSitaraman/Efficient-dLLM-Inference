...
The operating frontier is controlled by the verifier mode, the drafter shortcut, the draft horizon $K_{\text{draft}}$, and the composition strength $\gamma$.
...
We sweep routing/verifier settings, drafter cache aggressiveness, draft horizon $K_{\text{draft}}$, and policy temperature $\tau_\pi$.
...
\paragraph{Metrics.}
Accuracy (exact match / pass@1), throughput (tokens/sec), NFEs, verifier agreement $\bar{\alpha}$, stable-cache fraction $\bar{\kappa}$, drafter-cache reset rate, cache invalidation/thrashing, effective speedup vs.\ verifier-only decoding, and refresh ratio $\frac{1}{T}\sum_t |\cQ_t|/L$.
